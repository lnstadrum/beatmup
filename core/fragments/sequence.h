	...
	namespace Fragments {
		...
		*/
		...
		protected:
			...
			*/
			...
			protected:
				...
				Pointer(Sequence& sequence, int time, bool writing);
				~Pointer();
			...
			public:
				...
				*/
				...
				inline void jump(int time)				{ moveTo(currentTime + time); }
				inline int getTime() const				{ return currentTime; }
		...
		private:
			...
			std::vector<int> cumTimes;				//!< cumulative sum of fragment lengths, starts from 0, of N+1 entries (N = num. of fragments)
			...
			*/
			int findFragment(int time) const;
			...
			*/
			void splitFragment(int index, int delta);
		...
		protected:
			...
			*/
			void concatenate(Fragment& fragment, int offset, int length);
		...
		public:
			inline const int getLength() const { return cumTimes.back(); }
			...
			*/
			void shrink(int timeLeft, int timeRight);
			...
			*/
			Sequence* copy(int fromTime, int toTime) const;
			...
			*/
			virtual void insert(const Sequence& sequence, int time);

			/**
				Erases a part of a sequence between two given time moments
			*/
			void remove(int fromTime, int toTime);
						
			void split(int time, Sequence* left, Sequence* right);
		...
		public:
			T* copy(int fromTime, int toTime) const { return (T*)Sequence::copy(fromTime, toTime); }
		};
	}
}